...
-(void)SureTap:(UIButton *)sender
{
    ...
    self.NewPasswordTextField.placeholder = @"请输入密码";
    ...
    //可见密码按钮
    self.seeButton = [[UIButton alloc]initWithFrame:CGRectMake(LRYScreenpW(675), LRYScreenpH(476), LRYScreenpW(31), LRYScreenpH(46))];
    ...
    [self.updateButton setTitle:@"绑定学生" forState:UIControlStateNormal];
    ...
    [self.ValidataButton setTitle:@"重新发送(59s)" forState:UIControlStateNormal];
    [self.ValidataButton setTitleColor:LRYUIColorFromRGB(0x4faf2a, 0.5) forState:UIControlStateNormal];
    self.ValidataButton.userInteractionEnabled = NO;
...
-(void)finishTap:(UIButton *)sender
{
    ...
    self.alertbotTextView.text = @"      您将完成注册，是否确认注册";
